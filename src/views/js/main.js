...
// Empty array element to hold all of the pizza movers
var movers = [];
...
var resizePizzas = function(size) {
    ...
    // Deleted changeSliderLabel and moved functionality into changePizzaSizes

    // Deleted the determineDx function as it is unnecessary.

    // Iterates through pizza elements on the page and changes their widths
    function changePizzaSizes(size) {
        // According to several websites, getElementsByClassName is more efficient.
        var pizzas = document.getElementsByClassName('randomPizzaContainer');

        // Grab the pizzaSize element.
...
function updatePositions() {
    ...
    scTop = document.body.scrollTop / 1250;
    for (var i = 0; i < movers.length; i++) {
        var phase = Math.sin(scTop + (i % 5));
        var trans = -movers[i].basicLeft + 1000 * phase + 'px';
        movers[i].style.transform = 'translateX(' + trans + ') translateZ(0)';
...
/**
 * The idea for onScroll was reviewed viewed on Paul Lewis' post at:
 *   http://www.html5rocks.com/en/tutorials/speed/animations/
 *
 *   To prevent updatePositions from running on every scroll event, the 'ticking' variable
 *   is used to only running requestAnimationFrame when the previous execution has completed.
 */
function onScroll() {
    if (!ticking) {
        // scTop = document.body.scrollTop / 1250;
...
// runs onScroll when on scroll event occurs
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // No need for 200 pizzas as only 32 are seen onscreen at any given time.
    for (var i = 0; i < 31; i++) {
        var elem = document.createElement('img');
        elem.className = 'mover';
        elem.src = "images/pizza_73w.png";
        elem.basicLeft = (i % cols) * s;
        elem.style.top = (Math.floor(i / cols) * s) + 'px';
        document.querySelector("#movingPizzas1").appendChild(elem);
    }
    // Instead of repeatedly capturing the 'mover' pizzas, capture them
    // one-time for use later. Created the variable 'movers' at the top of the script
